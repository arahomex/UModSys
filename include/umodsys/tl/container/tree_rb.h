...
public:
  ...
  //
  inline const Self& operator++(int x) UMODSYS_NOTHROW() { node = next(node); return *this; }
  inline const Self& operator--(int x) UMODSYS_NOTHROW() { node = prev(node); return *this; }
  //
  tValue& operator*(void) const UMODSYS_NOTHROW();
  tValue* operator->(void) const UMODSYS_NOTHROW();
  //
